...
testthat::test_that("check vm_p_circum result assertions", {
  ...
  expect_message(vm_p_comp(vector_landscape, "class"), "MULTIPOLYGON geometry provided")
})
...
testthat::test_that("check vm_p_circum result structure", {
  ...
  expect_true(!is.na(vm_p_comp(squaretxt, "class")$class))
  expect_equal(
    nrow(vector_patches |> dplyr::inner_join(vm_p_comp(vector_patches, "class", "patch"), by = c("patch" = "id"))),
    nrow(vector_patches)
  )
  expect_true(all(
    vector_patches |> dplyr::inner_join(vm_p_comp(vector_patches, "class", "patch"), by = c("patch" = "id")) |> 
      dplyr::mutate(same_class = class.x == class.y) |> dplyr::pull(same_class)
  ))
  expect_type(vm_p_comp(square, "class")$class, "character")
  expect_type(vm_p_comp(square, "class")$id, "character")
  expect_type(vm_p_comp(square, "class")$value, "double")
})
