...
testthat::test_that("check vm_p_circum result structure", {
  ...
  expect_equal(
    nrow(vector_patches |> dplyr::inner_join(vm_p_comp(vector_patches, "class"), by = c("patch" = "id"))),
    nrow(vector_patches)
  )
  expect_true(all(
    vector_patches |> dplyr::inner_join(vm_p_comp(vector_patches, "class"), by = c("patch" = "id")) |> 
