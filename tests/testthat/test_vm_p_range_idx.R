...
testthat::test_that("check vm_p_range_idx result assertions", {
  expect_error(vm_p_range_idx(vector_patches |> sf::st_centroid(), "class"))
...
testthat::test_that("check vm_p_range_idx result strcuture", {
  ...
  expect_equal(
    nrow(vector_patches |> dplyr::inner_join(vm_p_range_idx(vector_patches, "class"), by = c("patch" = "id"))),
    nrow(vector_patches)
  )
  expect_true(all(
    vector_patches |> dplyr::inner_join(vm_p_range_idx(vector_patches, "class"), by = c("patch" = "id")) |> 
      dplyr::mutate(same_class = class.x == class.y) |> dplyr::pull(same_class)
  ))
  expect_type(vm_p_range_idx(square, "class")$value, "double")
})
