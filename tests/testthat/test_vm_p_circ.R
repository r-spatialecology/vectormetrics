...
testthat::test_that("check vm_p_circ result assertions", {
  expect_error(vm_p_circ(vector_patches |> sf::st_centroid(), "class"))
  expect_message(vm_p_circ(vector_landscape, "class"), "MULTIPOLYGON geometry provided")
})

testthat::test_that("check vm_p_circ result structure", {
  expect_s3_class(vm_p_circ(square, "class"), "tbl_df")
  expect_equal(ncol(vm_p_circ(square, "class")), 5)
