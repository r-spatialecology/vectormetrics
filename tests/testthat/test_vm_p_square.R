...
testthat::test_that("check vm_p_square result assertions", {
  ...
  expect_message(vm_p_square(vector_landscape, "class"), "MULTIPOLYGON geometry provided")
})
...
testthat::test_that("check vm_p_square result structure", {
  ...
  expect_equal(
    nrow(vector_patches |> dplyr::inner_join(vm_p_square(vector_patches, "class"), by = c("patch" = "id"))),
    nrow(vector_patches)
  )
  expect_true(all(
    vector_patches |> dplyr::inner_join(vm_p_square(vector_patches, "class"), by = c("patch" = "id")) |> 
