...
testthat::test_that("check vm_p_exchange result structure", {
  ...
  expect_equal(
    nrow(vector_patches |> dplyr::inner_join(vm_p_exchange(vector_patches, "class", "patch"), by = c("patch" = "id"))),
    nrow(vector_patches)
  )
  expect_true(all(
    vector_patches |> dplyr::inner_join(vm_p_exchange(vector_patches, "class", "patch"), by = c("patch" = "id")) |> 
      dplyr::mutate(same_class = class.x == class.y) |> dplyr::pull(same_class)
  ))
  expect_type(vm_p_exchange(square, "class")$class, "character")
  expect_type(vm_p_exchange(square, "class")$id, "character")
